...
my $f_vcf_tcga_hotspot = $script_dir."/hotspot.tcga.driver.chr.pos.tsv";
...
sub bsub_rc{
    ...
    print RC "$bamrc -q 10 -b 10 \${TBAM} -f $h38_REF -l $f_vcf_tcga_hotspot > $f_rc_t_out","\n";  
    print RC "$bamrc -q 10 -b 10 \${NBAM} -f $h38_REF -l $f_vcf_tcga_hotspot > $f_rc_n_out","\n";
    print RC "     ".$run_script_path."bamReadcount2vaf.pl -s $sample_name -l $f_mut_tcga_hotspot $f_rc_t_out > $f_vaf_t_out","\n"; 
    print RC "     ".$run_script_path."bamReadcount2vaf.pl -s $sample_name -l $f_mut_tcga_hotspot $f_rc_n_out > $f_vaf_n_out","\n"; 
    close RC;
    my $sh_file=$job_files_dir."/".$current_job_file;
